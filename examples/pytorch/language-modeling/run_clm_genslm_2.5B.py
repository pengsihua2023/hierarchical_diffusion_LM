...
def main():
    ...
    a = torch.load('/lus/eagle/projects/CVD-Mol-AI/model_checkpoints/patric_2B/last.pt')['state_dict']
    ...
    if not os.path.isfile(model_args.cl_model_name_or_path):
        import glob
        filename = glob.glob(os.path.join(model_args.cl_model_name_or_path, '*pt'))[0]
        model_args.cl_model_name_or_path = filename
    print (f'Loading CL Model from {model_args.cl_model_name_or_path}')

    state_dict = torch.load(model_args.cl_model_name_or_path)
    new_dict = {}
    ...
    for n, p in model.named_parameters():
        if 'cl_model' in n:
            p.requires_grad = False
    proj_model = nn.Linear(model_args.latent_dim, model.gpt_neox.embed_in.weight.shape[1])
